...
using the phonon spectral energy density (SED, referred to as $\Phi$). 
\nomenclature[A]{SED}{spectral energy density}
\nomenclature[G]{$\Phi$}{normal mode spectral energy density}
An alternative expression for 
the phonon SED (referred to as $\Phi'$), was recently proposed but has not 
\nomenclature[G]{$\Phi'$}{proposed alternative spectral energy density}
...
\nomenclature[A]{CNT}{carbon nanotube}
...
\nomenclature[A]{SW}{Stillinger-Weber}
...
\nomenclature[A]{REBO}{reactive empirical bond order}
...
\nomenclature[G]{$\pmb{\kappa}$}{wavevector} 
...
$\omega_0\kv$ 
\nomenclature[G]{$\omega_0\kv$}{anharmonic phonon frequency}
and linewidths 
$\Gamma \kv$ 
\nomenclature[G]{$\Gamma \kv$}{phonon linewidth}
(one for each polarization, $\nu$). 
\nomenclature[G]{$\nu$}{polarization}
The $C_0 \kv$ terms 
are mode-dependent constants. 
\nomenclature[V]{$C_0 \kv$}{mode-dependent Lorentzian constants}
For simplicity, we refer to $\Phi(\pmb{\kappa},\omega)$ as $\Phi$. 
The kinetic energy normal mode coordinate, $\dot{q}\kvt$, is 
\nomenclature[V]{$\dot{q}\kvt$}{normal mode coordinate time derivative}
...
\nomenclature[V]{$e\kvba$}{normal mode eigenvector}
...
\nomenclature[V]{$m_b$}{mass of the $b^{\textrm{th}}$ atom in the unit cell}
$\mathbf{r}_0\ab{l}{0}$ is the equilibrium position vector of the 
\nomenclature[V]{$\mathbf{r}_0\ab{l}{0}$}{equilibrium position vector of the $l^{\textrm{th}}$ unit cell}
\nomenclature[V]{$l$}{unit cell label}
\nomenclature[V]{$b$}{atom in the unit cell label}
\nomenclature[V]{$t$}{time}
\nomenclature[G]{$\alpha$}{cartesian component label}
...
%\nomenclature[V]{$\dot{u}_{\alpha}\lbt$}{$\alpha$-component of the velocity of the
%$b^{\textrm{th}}$ atom in the $l^{\textrm{th}}$ unit cell at time $t$}
...
\nomenclature[G]{$\tau \kv$}{phonon lifetime}
In practice, $\tau_0$ should 
\nomenclature[G]{$\tau_0$}{time integration limit}
...
\nomenclature[V]{$q\kvt$}{normal mode coordiante}
...
\nomenclature[V]{$E\kvt$}{normal mode total energy}
...
\nomenclature[G]{$\Theta$}{spatial and temporal phase factor}
...
\nomenclature[V]{$D$}{number of spatial dimensions}
is a collection of 
points with
positions
...
\nomenclature[V]{$\mathbf{a}_{\alpha}$}{lattice vector}
an integer 
\cite{dove_introduction_1993}.
\nomenclature[V]{$N_{\alpha}$}{lattice vector integer} 
...
\nomenclature[V]{$\mathbf{r}_0\ab{0}{b}$}{equilibrium position of the 
$b^{\textrm{th}}$ atom in 
the unit cell relative to $\mathbf{r}_0\ab{l}{0}$} 
...
\nomenclature[V]{$\mathbf{b}_{\alpha}$}{reciprocal lattice vector}
\nomenclature[V]{$n_{\alpha}$}{reciprocal lattice vector integer}
...
% \nomenclature[G]{$\pi$}{the number pi}
...
\nomenclature[V]{$k_{\mathbf{n}}$}{thermal conductivity in direction $\mathbf{n}$}
...
\nomenclature[V]{$c_{ph}$}{phonon volumetric specific heat}
\nomenclature[V]{${v}_{g,\mathbf{n}}$}{component of the group velocity 
vector in direction $\mathbf{n}$} 
Since the systems 
...
\nomenclature[V]{$k_{\mathrm{B}}$}{Boltzmann's constant} 
As temperature increases, anharmonicity causes 
the mode specific 
...
scale as $N_a$.
\nomenclature[A]{GULP}{General Utility Lattice Program}
...
\nomenclature[V]{$T_{\mathrm{melt}}$}{melting temperature}
Here, we limit the 
temperature to below half the melting temperature for the three systems 
studied (argon, 
silicon, and CNT).
\nomenclature[V]{$a$}{lattice constant}
...
\nomenclature[A]{$NVE$}{constant number of atoms, system volume, and 
total system energy ensemble}
